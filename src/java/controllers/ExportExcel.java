...
public class ExportExcel extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String[] headers = {"Product ID", "Product Code", "Product Name", "Category", "Price", "Quantity", "Description", "CreatedDate", "ExpiredDate", "UpdateDate","Image","cate"};
        ...
        for (Product product : products) {
            ...
            if (product.getUpdateDate() != null) {
                ...
                updateDateCell.setCellStyle(dateCellStyle);            // Áp dụng định dạng ngày tháng
                        ...
                        String imagePath = "path/to/img-sanpham/" + product.getImage(); // Đường dẫn đến file ảnh của sản phẩm
            try (InputStream inputStream = new FileInputStream(imagePath)) {
                // Đọc tệp ảnh thành byte[]
                byte[] imageBytes = IOUtils.toByteArray(inputStream);

                // Thêm hình ảnh vào Workbook
                int pictureIdx = workbook.addPicture(imageBytes, Workbook.PICTURE_TYPE_PNG); // Hoặc PICTURE_TYPE_JPEG cho file JPG

                // Tạo Drawing để vẽ ảnh
                Drawing drawing = sheet.createDrawingPatriarch();
                
                // Đặt vị trí ảnh trong Excel
                ClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, 9, rowNum - 1, 10, rowNum); // 9: cột thứ 10, rowNum-1: dòng chứa ảnh
            Picture picture = drawing.createPicture(anchor, pictureIdx);
            picture.resize(); // Tự động điều chỉnh kích thước ảnh
        } catch (Exception e) {
